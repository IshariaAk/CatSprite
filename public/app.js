 ...
 * LICENSE file in the root directory of this source tree.
 */
var l=__webpack_require__(456),n=60103,p=60106;exports.Fragment=60107;exports.StrictMode=60108;exports.Profiler=60114;var q=60109,r=60110,t=60112;exports.Suspense=60113;var u=60115,v=60116;
...
function Sidebar(_ref) {
  ...
  const [draggedItem, setDraggedItem] = (0,react.useState)(null);
  const [showHello, setShowHello] = (0,react.useState)(false);
  const handleDragEnd = () => {
    setDraggedItem(null);
  };
  const handleDragStart = (item, rotationDirection) => event => {
    const id = "".concat(item.type, "-").concat(Date.now());
    const item_Id = {
      ...item,
      id: id,
      rotationDirection
...
function MidArea(_ref) {
  ...
  const [rotationRight, setRotationRight] = (0,react.useState)(0);
  const [catPosition, setCatPosition] = (0,react.useState)({
    x: 0,
    y: 0
  });
  const handleRotate1 = () => {
  ...
  const handleDrop = event => {
    ...
    // if (droppedItem.type === 'SAY') {
    //   // handleSayHello && handleSayHello(droppedItem.text);
    // }
  ...
  const handlePlay = () => {
    droppedItems.forEach(item => {
      if (item.type === 'MOVE') {
        ...
        // console.log(`Turn ${item.degrees} degrees ${item.rotationDirection}`);
        onleftRotate();
  ...
  /*#__PURE__*/
  // <div className="flex flex-row flex-wrap bg-blue-500 text-white px-2 py-1 my-2 text-sm cursor-pointer" key={item.id}>
  react.createElement("div", {
    className: "flex flex-row flex-wrap text-white px-2 py-1 my-2 text-sm cursor-pointer \n        ".concat(item.type === 'SAY' ? 'bg-yellow-500' : 'bg-blue-500'),
    style: {
      borderRadius: '8px'
    },
    key: item.id
  }, item.type == 'SAY' ? 'Say ' + item.text + ' for 2 seconds' : item.type, " -", item.type === 'TURN' && /*#__PURE__*/react.createElement("div", {
    className: "flex"
  }, item.rotationDirection === 'clockwise' && /*#__PURE__*/react.createElement(Icon, {
    name: "undo",
    size: 15,
    className: "text-white mx-2"
  }), item.rotationDirection === 'anticlockwise' && /*#__PURE__*/react.createElement(Icon, {
    name: "redo",
    size: 15,
    className: "text-white mx-2"
  }), item.degrees && /*#__PURE__*/react.createElement("span", null, item.degrees, " degrees")), item.steps && /*#__PURE__*/react.createElement("span", null, item.steps, " steps"))), /*#__PURE__*/react.createElement("button", {
    onClick: handlePlay
...
function CatSprite(_ref) {
  ...
  // console.log("position catsprite",rotation,"dddd",position,"rotationRight",rotationRight,"showHello",showHello)
  const style = {
    position: "relative",
    left: "".concat(position.x, "px"),
    top: "".concat(position.y, "px"),
    transform: "rotate(".concat(rotation, "deg) ").concat(rotationRight ? "rotate(".concat(rotationRight, "deg)") : '')
  };
  const cloudStyle = {
    position: "relative",
    left: "50%",
    top: "".concat(position.x + 20, "px"),
  ...
  return /*#__PURE__*/react.createElement(react.Fragment, null, /*#__PURE__*/react.createElement("div", {
    style: cloudStyle
  }, showHello && /*#__PURE__*/react.createElement("p", {
    style: {
      margin: 10
    }
...
function PreviewArea(_ref) {
  ...
  // console.log("Preview onleftRotate",onleftRotate,"onRightRotate",onRightRotate)
  (0,react.useEffect)(() => {
  ...
  //  console.log("movr pre",onMove)
  //  console.log("movr pre",catPosition)
  //  console.log("rotate pre ",onRightRotate)

  return /*#__PURE__*/react.createElement("div", {
    className: "flex-none h-full p-4 "
...
function App() {
  ...
  const [droppedItem, setDroppedItem] = (0,react.useState)(null);
  const [rotation, setRotation] = (0,react.useState)(0);
  const [rotationRight, setRotationRight] = (0,react.useState)(0);
  const [showHello, setShowHello] = (0,react.useState)(false);
  const [showMessage, setShowMessage] = (0,react.useState)('');
  const handleMove = steps => {
    setCatPosition(prevPosition => ({
